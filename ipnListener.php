...
$fp = fsockopen('ssl://www.sandbox.paypal.com', 443, $errno, $errstr, 30);
 ...
 while (!feof($fp)) {
	...
	if (strcmp ($res, "VERIFIED\r\n") == 0) { 
		...
		error_log("test invoice: ".$invoice, 3, "/var/www/html/myerrors.log");
		foreach($cartOrder as $car){
			if ($car['oid']==$invoice){
				if($car['transactionid']==$_POST['txn_id']){
		...
		*/
		...
		do{
			error_log('the i='.$i);
			array_push($list, $_POST['item_name'.$i], (int)($_POST['quantity'.$i]));
			...
			$prod1 = $_POST['item_name'.$i].'&'.($_POST['quantity'.$i]).'&'.(int)($_POST['mc_gross_'.$i]).'|';
		...
		$cart_info = substr($cart_information, 0, -1);
		$priceStr=$priceList_combine;
		$totalPrice=(int)($_POST['mc_gross']);
		error_log($Currency.$MerEmail.$salt.$cart_info.$totalPrice, 3, "/var/www/html/errors.log");
		
		/*
		generate a $digest using hash function like sha1, and compare $digest with $digestOld 
		*/
		$digest = sha1($Currency.$MerEmail.$salt.$cart_info.$totalPrice);

        if ($digest==$digestOld)
		{	
			$q = $db->prepare("UPDATE orders SET transactionid = ? WHERE oid = ?");
